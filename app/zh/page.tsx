...
export default function ZhPage() {
  ...
  const stats = [
    { value: zh.stat_free_val,  label: zh.stat_free_label },
    { value: '\$9.99',           label: zh.stat_price_label },
    { value: '4',               label: zh.stat_levels_label },
    { value: '∞',               label: zh.stat_reach_label },
  ];

  const steps = [
    { num: '01', title: zh.step_01_title, desc: zh.step_01_desc },
    { num: '02', title: zh.step_02_title, desc: zh.step_02_desc },
    { num: '03', title: zh.step_03_title, desc: zh.step_03_desc },
    { num: '04', title: zh.step_04_title, desc: zh.step_04_desc },
  ];

  const ladder = [
    { tier: 'Entry',    color: teal,   pts: zh.ladder_entry_pts,    desc: zh.ladder_entry_desc },
    { tier: 'Rising',   color: blue,   pts: zh.ladder_rising_pts,   desc: zh.ladder_rising_desc },
    { tier: 'Featured', color: orange, pts: zh.ladder_featured_pts, desc: zh.ladder_featured_desc },
    { tier: 'Top Tier', color: gold,   pts: zh.ladder_top_pts,      desc: zh.ladder_top_desc },
  ];

  const plans = [
    {
      name:     zh.plan_trial_name,
      price:    zh.plan_trial_price,
      period:   zh.plan_trial_period,
      badge:    '',
      badgeColor: '',
      color:    teal,
      features: [zh.plan_trial_f1, zh.plan_trial_f2, zh.plan_trial_f3, zh.plan_trial_f4],
      cta:      zh.plan_trial_cta,
      disabled: false,
    },
    {
      name:     zh.plan_arena_name,
      price:    zh.plan_arena_price,
      period:   zh.plan_arena_period,
      badge:    zh.plan_arena_badge,
      badgeColor: orange,
      color:    orange,
      features: [zh.plan_arena_f1, zh.plan_arena_f2, zh.plan_arena_f3, zh.plan_arena_f4, zh.plan_arena_f5, zh.plan_arena_f6],
      cta:      zh.plan_arena_cta,
      disabled: false,
    },
    {
      name:     zh.plan_pro_name,
      price:    zh.plan_pro_price,
      period:   zh.plan_pro_period,
      badge:    zh.plan_pro_badge,
      badgeColor: purple,
      color:    purple,
      features: [zh.plan_pro_f1, zh.plan_pro_f2, zh.plan_pro_f3, zh.plan_pro_f4, zh.plan_pro_f5, zh.plan_pro_f6],
      cta:      zh.plan_pro_cta,
      disabled: true,
    },
    {
      name:     zh.plan_deluxe_name,
      price:    zh.plan_deluxe_price,
      period:   zh.plan_deluxe_period,
      badge:    zh.plan_deluxe_badge,
      badgeColor: gold,
      color:    gold,
      features: [zh.plan_deluxe_f1, zh.plan_deluxe_f2, zh.plan_deluxe_f3, zh.plan_deluxe_f4, zh.plan_deluxe_f5, zh.plan_deluxe_f6],
      cta:      zh.plan_deluxe_cta,
      disabled: true,
    },
  ];
    ...
    <>
      ...
      <nav style={navStyle}>
        ...
        <div style={navLinksStyle}>
          <LanguageSwitcher />
          <a href="/login" style={signInStyle} className="sign-in-link">{zh.nav_signin}</a>
          <a href="/login#start" style={ctaButtonStyle} className="cta-btn">{zh.nav_start}</a>
        </div>
      </nav>
      ...
      <div style={{ background: bg, color: white, fontFamily: 'system-ui, -apple-system, sans-serif', minHeight: '100vh', paddingTop: '60px', overflowX: 'hidden' }}>
        ...
        <section style={{ position: 'relative', padding: 'clamp(80px, 12vw, 160px) 0 clamp(60px, 8vw, 100px)', textAlign: 'center', overflow: 'hidden' }}>
          ...
          <div style={{ ...sectionStyle, position: 'relative' }}>
            ...
            <p style={{
              ...
              {zh.hero_sub}<br />
              <span style={{ color: white, fontWeight: 600 }}>{zh.hero_trial}</span>
            </p>
        ...
        <section style={{ padding: 'clamp(60px, 8vw, 100px) 0' }}>
          <div style={sectionStyle}>
            <div style={{ textAlign: 'center', marginBottom: '40px' }}>
              <div style={sectionHeadStyle}>{zh.partner_section_label}</div>
            </div>
                ...
                <div style={{ flex: '1 1 320px' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '8px' }}>
                    ...
                    <div>
                      ...
                      <div style={{ fontSize: '11px', fontWeight: 700, color: gold, letterSpacing: '2px', textTransform: 'uppercase' }}>{zh.partner_section_label}</div>
                    </div>
                  </div>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(120px, 1fr))', gap: '12px', marginBottom: '20px' }}>
                    {[
                      { v: '2.1M+', l: zh.partner_users_label },
                      { v: '148K',  l: zh.partner_sellers_label },
                      { v: '173K+', l: zh.partner_tx_label },
                      { v: '\$0.17', l: zh.partner_price_label },
                    ].map((st, i) => (
        ...
        <section style={{ padding: 'clamp(60px, 8vw, 100px) 0', borderTop: `1px solid ${border}` }}>
          <div style={sectionStyle}>
            <div style={{ textAlign: 'center', marginBottom: '56px' }}>
              <div style={sectionHeadStyle}>{zh.how_section_label}</div>
              <h2 style={{ ...h2Style, marginBottom: 0 }}>{zh.how_title}</h2>
            </div>
        ...
        <section style={{ padding: 'clamp(60px, 8vw, 100px) 0', borderTop: `1px solid ${border}` }}>
          <div style={sectionStyle}>
            <div style={{ textAlign: 'center', marginBottom: '48px' }}>
              <div style={sectionHeadStyle}>{zh.ladder_section_label}</div>
              <h2 style={h2Style}>{zh.ladder_title}</h2>
              <p style={{ color: muted, fontSize: '15px', maxWidth: '460px', margin: '0 auto' }}>{zh.ladder_sub}</p>
            </div>
        ...
        <section style={{ padding: 'clamp(60px, 8vw, 100px) 0', borderTop: `1px solid ${border}` }}>
          <div style={sectionStyle}>
            <div style={{ textAlign: 'center', marginBottom: '48px' }}>
              <div style={sectionHeadStyle}>{zh.pricing_section_label}</div>
              <h2 style={h2Style}>{zh.pricing_title}</h2>
              <p style={{ color: muted, fontSize: '15px' }}>{zh.pricing_sub}</p>
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(230px, 1fr))', gap: '16px' }}>
              {plans.map((plan, i) => (
                <div key={i} className="pricing-card" style={{ background: card, border: `1px solid ${plan.name === zh.plan_arena_name ? plan.color + '66' : border2}`, borderRadius: '14px', padding: 'clamp(20px, 2.5vw, 32px)', display: 'flex', flexDirection: 'column', position: 'relative', transition: 'border-color 0.2s, transform 0.2s', boxShadow: plan.name === zh.plan_arena_name ? `0 0 40px ${plan.color}22` : 'none' }}>
                  {plan.badge && (
        ...
        <section style={{ padding: 'clamp(80px, 10vw, 140px) 0', borderTop: `1px solid ${border}`, position: 'relative', overflow: 'hidden', textAlign: 'center' }}>
          ...
          <div style={{ ...sectionStyle, position: 'relative' }}>
            <div style={sectionHeadStyle}>{zh.final_section_label}</div>
            <h2 style={{ ...h2Style, fontSize: 'clamp(36px, 5vw, 60px)' }}>{zh.final_title}</h2>
            <p style={{ color: muted, fontSize: '15px', marginBottom: '36px' }}>{zh.final_sub}</p>
            ...
            <div style={{ marginTop: '20px' }}>
              <a href="/login#signin" style={{ fontSize: '14px', color: muted, textDecoration: 'none' }} className="sign-in-link">
                <span style={{ color: white, fontWeight: 600 }}>{zh.final_signin}</span>
              </a>
            </div>
          </div>
        </section>

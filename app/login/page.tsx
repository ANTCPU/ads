...
export default function Page() {
  ...
  useEffect(() => {
    fetch('/api/doorbell', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ page: '/login', ref: document.referrer || 'direct', ts: new Date().toISOString(), ua: navigator.userAgent }),
    }).catch(() => {});
  ...
  return (
    <div style={{ minHeight: '100vh', background: bg, color: white, fontFamily: 'system-ui, sans-serif' }}>
      ...
      <nav style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '1rem 2rem', borderBottom: `1px solid ${border}` }}>
        ...
        <button onClick={() => setVaultOpen(true)} style={{ background: 'none', border: `1px solid ${border}`, color: muted, borderRadius: '8px', padding: '0.4rem 1rem', cursor: 'pointer', fontSize: '0.85rem' }}>
          🔒 Sign In with Vault
        </button>
      </nav>
      ...
      <div style={{ maxWidth: '480px', margin: '0 auto', padding: '3rem 1.5rem 1rem' }}>
        ...
        <div style={{ background: card, border: `1px solid ${border}`, borderRadius: '16px', padding: '2rem' }}>
          {isBrand ? (
            <>
              ...
              <button style={btn(!!email.trim())} onClick={handleBrandCTA} disabled={!email.trim()}>
            ...
            <>
              ...
              {step === 2 && (
                <>
                  ...
                  <button style={btn(true)} onClick={handleSubmit}>{loading ? 'Launching...' : 'Launch My Ad →'}</button>
                </>
              )}
            </>
          )}
        ...
        <p style={{ color: muted, fontSize: '0.78rem', textAlign: 'center', marginTop: '1.5rem' }}>
          ...
          <button onClick={() => setVaultOpen(true)} style={{ background: 'none', border: 'none', color: accent, cursor: 'pointer', fontSize: '0.78rem', padding: 0 }}>

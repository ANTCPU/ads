...
export default function ItPage() {
  ...
  const stats = [
    { value: it.stat_free_val,  label: it.stat_free_label },
    { value: '\$9.99',           label: it.stat_price_label },
    { value: '4',               label: it.stat_levels_label },
    { value: '∞',               label: it.stat_reach_label },
  ];

  const steps = [
    { num: '01', title: it.step_01_title, desc: it.step_01_desc },
    { num: '02', title: it.step_02_title, desc: it.step_02_desc },
    { num: '03', title: it.step_03_title, desc: it.step_03_desc },
    { num: '04', title: it.step_04_title, desc: it.step_04_desc },
  ];

  const ladder = [
    { tier: 'Entry',    color: teal,   pts: it.ladder_entry_pts,    desc: it.ladder_entry_desc },
    { tier: 'Rising',   color: blue,   pts: it.ladder_rising_pts,   desc: it.ladder_rising_desc },
    { tier: 'Featured', color: orange, pts: it.ladder_featured_pts, desc: it.ladder_featured_desc },
    { tier: 'Top Tier', color: gold,   pts: it.ladder_top_pts,      desc: it.ladder_top_desc },
  ];

  const plans = [
    {
      name:     it.plan_trial_name,
      price:    it.plan_trial_price,
      period:   it.plan_trial_period,
      badge:    '',
      badgeColor: '',
      color:    teal,
      features: [it.plan_trial_f1, it.plan_trial_f2, it.plan_trial_f3, it.plan_trial_f4],
      cta:      it.plan_trial_cta,
      disabled: false,
    },
    {
      name:     it.plan_arena_name,
      price:    it.plan_arena_price,
      period:   it.plan_arena_period,
      badge:    it.plan_arena_badge,
      badgeColor: orange,
      color:    orange,
      features: [it.plan_arena_f1, it.plan_arena_f2, it.plan_arena_f3, it.plan_arena_f4, it.plan_arena_f5, it.plan_arena_f6],
      cta:      it.plan_arena_cta,
      disabled: false,
    },
    {
      name:     it.plan_pro_name,
      price:    it.plan_pro_price,
      period:   it.plan_pro_period,
      badge:    it.plan_pro_badge,
      badgeColor: purple,
      color:    purple,
      features: [it.plan_pro_f1, it.plan_pro_f2, it.plan_pro_f3, it.plan_pro_f4, it.plan_pro_f5, it.plan_pro_f6],
      cta:      it.plan_pro_cta,
      disabled: true,
    },
    {
      name:     it.plan_deluxe_name,
      price:    it.plan_deluxe_price,
      period:   it.plan_deluxe_period,
      badge:    it.plan_deluxe_badge,
      badgeColor: gold,
      color:    gold,
      features: [it.plan_deluxe_f1, it.plan_deluxe_f2, it.plan_deluxe_f3, it.plan_deluxe_f4, it.plan_deluxe_f5, it.plan_deluxe_f6],
      cta:      it.plan_deluxe_cta,
      disabled: true,
    },
  ];
    ...
    <>
      ...
      <nav style={navStyle}>
        ...
        <div style={navLinksStyle}>
          <LanguageSwitcher />
          <a href="/login" style={signInStyle} className="sign-in-link">{it.nav_signin}</a>
          <a href="/login#start" style={ctaButtonStyle} className="cta-btn">{it.nav_start}</a>
        </div>
      </nav>
      ...
      <div style={{ background: bg, color: white, fontFamily: 'system-ui, -apple-system, sans-serif', minHeight: '100vh', paddingTop: '60px', overflowX: 'hidden' }}>
        ...
        <section style={{ position: 'relative', padding: 'clamp(80px, 12vw, 160px) 0 clamp(60px, 8vw, 100px)', textAlign: 'center', overflow: 'hidden' }}>
          ...
          <div style={{ ...sectionStyle, position: 'relative' }}>
            ...
            <p style={{
              ...
              {it.hero_sub}<br />
              <span style={{ color: white, fontWeight: 600 }}>{it.hero_trial}</span>
            </p>
        ...
        <section style={{ padding: 'clamp(60px, 8vw, 100px) 0' }}>
          <div style={sectionStyle}>
            <div style={{ textAlign: 'center', marginBottom: '40px' }}>
              <div style={sectionHeadStyle}>{it.partner_section_label}</div>
            </div>
                ...
                <div style={{ flex: '1 1 320px' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '8px' }}>
                    ...
                    <div>
                      ...
                      <div style={{ fontSize: '11px', fontWeight: 700, color: gold, letterSpacing: '2px', textTransform: 'uppercase' }}>{it.partner_section_label}</div>
                    </div>
                  </div>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(120px, 1fr))', gap: '12px', marginBottom: '20px' }}>
                    {[
                      { v: '2.1M+', l: it.partner_users_label },
                      { v: '148K',  l: it.partner_sellers_label },
                      { v: '173K+', l: it.partner_tx_label },
                      { v: '\$0.17', l: it.partner_price_label },
                    ].map((st, i) => (
        ...
        <section style={{ padding: 'clamp(60px, 8vw, 100px) 0', borderTop: `1px solid ${border}` }}>
          <div style={sectionStyle}>
            <div style={{ textAlign: 'center', marginBottom: '56px' }}>
              <div style={sectionHeadStyle}>{it.how_section_label}</div>
              <h2 style={{ ...h2Style, marginBottom: 0 }}>{it.how_title}</h2>
            </div>
        ...
        <section style={{ padding: 'clamp(60px, 8vw, 100px) 0', borderTop: `1px solid ${border}` }}>
          <div style={sectionStyle}>
            <div style={{ textAlign: 'center', marginBottom: '48px' }}>
              <div style={sectionHeadStyle}>{it.ladder_section_label}</div>
              <h2 style={h2Style}>{it.ladder_title}</h2>
              <p style={{ color: muted, fontSize: '15px', maxWidth: '460px', margin: '0 auto' }}>{it.ladder_sub}</p>
            </div>
        ...
        <section style={{ padding: 'clamp(60px, 8vw, 100px) 0', borderTop: `1px solid ${border}` }}>
          <div style={sectionStyle}>
            <div style={{ textAlign: 'center', marginBottom: '48px' }}>
              <div style={sectionHeadStyle}>{it.pricing_section_label}</div>
              <h2 style={h2Style}>{it.pricing_title}</h2>
              <p style={{ color: muted, fontSize: '15px' }}>{it.pricing_sub}</p>
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(230px, 1fr))', gap: '16px' }}>
              {plans.map((plan, i) => (
                <div key={i} className="pricing-card" style={{ background: card, border: `1px solid ${plan.name === it.plan_arena_name ? plan.color + '66' : border2}`, borderRadius: '14px', padding: 'clamp(20px, 2.5vw, 32px)', display: 'flex', flexDirection: 'column', position: 'relative', transition: 'border-color 0.2s, transform 0.2s', boxShadow: plan.name === it.plan_arena_name ? `0 0 40px ${plan.color}22` : 'none' }}>
                  {plan.badge && (
        ...
        <section style={{ padding: 'clamp(80px, 10vw, 140px) 0', borderTop: `1px solid ${border}`, position: 'relative', overflow: 'hidden', textAlign: 'center' }}>
          ...
          <div style={{ ...sectionStyle, position: 'relative' }}>
            <div style={sectionHeadStyle}>{it.final_section_label}</div>
            <h2 style={{ ...h2Style, fontSize: 'clamp(36px, 5vw, 60px)' }}>{it.final_title}</h2>
            <p style={{ color: muted, fontSize: '15px', marginBottom: '36px' }}>{it.final_sub}</p>
            ...
            <div style={{ marginTop: '20px' }}>
              <a href="/login#signin" style={{ fontSize: '14px', color: muted, textDecoration: 'none' }} className="sign-in-link">
                <span style={{ color: white, fontWeight: 600 }}>{it.final_signin}</span>
              </a>
            </div>
          </div>
        </section>

// ============================================================
// app/internship/join/page.tsx
// Internship challenger signup — 3 steps
//
// Step 1: Name + Email + Country
// Step 2: Track — 💻 Dev or 📣 Marketing
// Step 3: Confirm + Submit → POST /api/internship/register
//
// ?track=dev or ?track=marketing pre-selects the track card.
// On success → /internship/success?name=...&track=...
//
// Mirrors /mapofpi/create-shop-ad pattern.
...
// ─── Country list — matches ad_signups.country values ────────
const COUNTRIES = [
...
const S = {
  ...
  input: { width: '100%', padding: '0.8rem 1rem', borderRadius: 8, border: '1px solid #d1d5db', fontSize: '1rem', boxSizing: 'border-box', outline: 'none' } as React.CSSProperties,
  btnPrimary: (disabled: boolean) => ({ width: '100%', background: disabled ? '#d1d5db' : '#2563eb', color: disabled ? '#9ca3af' : '#fff', border: 'none', padding: '0.9rem', borderRadius: 8, fontWeight: 700, fontSize: '1rem', cursor: disabled ? 'not-allowed' : 'pointer', transition: 'background 0.15s' }) as React.CSSProperties,
    ...
    <div style={{ display: 'flex', gap: 6, marginBottom: '2rem' }}>
      {[1, 2, 3].map(n => (
        <div key={n} style={{
          ...
          background: n <= step ? '#2563eb' : '#e5e7eb',
...
function JoinForm() {
  ...
  const [form,    setForm]    = useState({
    name: '', email: '', country: '', track: preTrack,
  });
  ...
  async function submit() {
    ...
    try {
      ...
      router.push(
        `/internship/success?name=${encodeURIComponent(form.name)}&track=${form.track}`
      );
    } catch (e: unknown) {
  ...
  if (step === 1) return (
    <div style={S.page}>
      ...
      <p style={{ color: '#6b7280', marginBottom: '2rem', fontSize: '0.95rem' }}>
        No CV. No experience required. Just show up.
      </p>
      ...
      <button style={{ ...S.btnPrimary(!step1Valid), marginTop: '1.75rem' }}
  ...
  if (step === 2) return (
    <div style={S.page}>
      ...
      <p style={{ color: '#6b7280', marginBottom: '2rem', fontSize: '0.95rem' }}>
        You are a brand. Your skills are your service.<br />
        One track only — choose what fits you best.
      </p>
        ...
        {[
          {
            ...
            icon:  '💻',
            label: 'Developer',
            desc:  'Build tools, automate workflows, write code. Ship real features to the Arena.',
          },
          {
            track: 'marketing',
            icon:  '📣',
            label: 'Marketer',
            desc:  'Run campaigns, build community, drive real reach. Represent brands in the Arena.',
          },
        ].map(t => (
          <button key={t.track} onClick={() => set('track', t.track)} style={{
            padding: '1.25rem 1.5rem', borderRadius: 12, cursor: 'pointer',
      ...
      <div style={{ display: 'flex', gap: '0.75rem', marginTop: '1.75rem' }}>
        ...
        <button style={{ ...S.btnPrimary(!step2Valid), flex: 2 }}
  ...
  return (
    <div style={S.page}>
      ...
      <div style={{ background: '#f9fafb', border: '1px solid #e5e7eb', borderRadius: 12, padding: '1.25rem', marginBottom: '1.5rem' }}>
        <div style={{ fontSize: '0.7rem', color: '#9ca3af', fontWeight: 700, letterSpacing: '0.08em', textTransform: 'uppercase', marginBottom: '0.75rem' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.4rem' }}>
          ...
          <div style={{ marginTop: '0.5rem', display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
            <span style={{ background: '#eff6ff', color: '#2563eb', padding: '0.25rem 0.75rem', borderRadius: 999, fontSize: '0.8rem', fontWeight: 700 }}>
              {trackLabel}
            </span>
            <span style={{ background: '#f0fdf4', color: '#16a34a', padding: '0.25rem 0.75rem', borderRadius: 999, fontSize: '0.8rem', fontWeight: 700 }}>
              ⭐ Founding Member
            </span>
            <span style={{ background: '#fafafa', color: '#6b7280', padding: '0.25rem 0.75rem', borderRadius: 999, fontSize: '0.8rem', fontWeight: 600 }}>
              Week 1 · Explorer
            </span>
          </div>
        </div>
      ...
      <p style={{ fontSize: '0.8rem', color: '#9ca3af', marginBottom: '1.5rem', lineHeight: 1.6 }}>
        ...
        <a href="https://antcpu.io/terms/" target="_blank" rel="noopener noreferrer" style={{ color: '#2563eb' }}>
          Terms of Participation
        </a>{' '}
        and{' '}
        <a href="https://antcpu.io/community-guidelines/" target="_blank" rel="noopener noreferrer" style={{ color: '#2563eb' }}>
          Community Guidelines
        </a>.
        This is a free, unpaid challenge — ~5 hours/week, fully remote.
      </p>

      {error && <div style={S.error}>{error}</div>}

      <div style={{ display: 'flex', gap: '0.75rem' }}>
        <button style={{ ...S.btnSecondary, flex: 1 }} onClick={() => setStep(2)} disabled={loading}>
          ← Back
        </button>
        <button style={{ ...S.btnPrimary(loading), flex: 2 }} onClick={submit} disabled={loading}>
...
export default function InternshipJoinPage() {
  return (
    <Suspense fallback={
      <div style={{ maxWidth: 520, margin: '0 auto', padding: '3rem 1.5rem', fontFamily: 'system-ui,sans-serif', color: '#9ca3af' }}>
        Loading...
      </div>
    }>
      <JoinForm />

...
export default function HiPage() {
  ...
  const stats = [
    { value: hi.stat_free_val,  label: hi.stat_free_label },
    { value: '\$9.99',           label: hi.stat_price_label },
    { value: '4',               label: hi.stat_levels_label },
    { value: '∞',               label: hi.stat_reach_label },
  ];

  const steps = [
    { num: '01', title: hi.step_01_title, desc: hi.step_01_desc },
    { num: '02', title: hi.step_02_title, desc: hi.step_02_desc },
    { num: '03', title: hi.step_03_title, desc: hi.step_03_desc },
    { num: '04', title: hi.step_04_title, desc: hi.step_04_desc },
  ];

  const ladder = [
    { tier: 'Entry',    color: teal,   pts: hi.ladder_entry_pts,    desc: hi.ladder_entry_desc },
    { tier: 'Rising',   color: blue,   pts: hi.ladder_rising_pts,   desc: hi.ladder_rising_desc },
    { tier: 'Featured', color: orange, pts: hi.ladder_featured_pts, desc: hi.ladder_featured_desc },
    { tier: 'Top Tier', color: gold,   pts: hi.ladder_top_pts,      desc: hi.ladder_top_desc },
  ];

  const plans = [
    {
      name:     hi.plan_trial_name,
      price:    hi.plan_trial_price,
      period:   hi.plan_trial_period,
      badge:    '',
      badgeColor: '',
      color:    teal,
      features: [hi.plan_trial_f1, hi.plan_trial_f2, hi.plan_trial_f3, hi.plan_trial_f4],
      cta:      hi.plan_trial_cta,
      disabled: false,
    },
    {
      name:     hi.plan_arena_name,
      price:    hi.plan_arena_price,
      period:   hi.plan_arena_period,
      badge:    hi.plan_arena_badge,
      badgeColor: orange,
      color:    orange,
      features: [hi.plan_arena_f1, hi.plan_arena_f2, hi.plan_arena_f3, hi.plan_arena_f4, hi.plan_arena_f5, hi.plan_arena_f6],
      cta:      hi.plan_arena_cta,
      disabled: false,
    },
    {
      name:     hi.plan_pro_name,
      price:    hi.plan_pro_price,
      period:   hi.plan_pro_period,
      badge:    hi.plan_pro_badge,
      badgeColor: purple,
      color:    purple,
      features: [hi.plan_pro_f1, hi.plan_pro_f2, hi.plan_pro_f3, hi.plan_pro_f4, hi.plan_pro_f5, hi.plan_pro_f6],
      cta:      hi.plan_pro_cta,
      disabled: true,
    },
    {
      name:     hi.plan_deluxe_name,
      price:    hi.plan_deluxe_price,
      period:   hi.plan_deluxe_period,
      badge:    hi.plan_deluxe_badge,
      badgeColor: gold,
      color:    gold,
      features: [hi.plan_deluxe_f1, hi.plan_deluxe_f2, hi.plan_deluxe_f3, hi.plan_deluxe_f4, hi.plan_deluxe_f5, hi.plan_deluxe_f6],
      cta:      hi.plan_deluxe_cta,
      disabled: true,
    },
  ];
    ...
    <>
      ...
      <nav style={navStyle}>
        ...
        <div style={navLinksStyle}>
          <LanguageSwitcher />
          <a href="/login" style={signInStyle} className="sign-in-link">{hi.nav_signin}</a>
          <a href="/login#start" style={ctaButtonStyle} className="cta-btn">{hi.nav_start}</a>
        </div>
      </nav>
      ...
      <div style={{ background: bg, color: white, fontFamily: 'system-ui, -apple-system, sans-serif', minHeight: '100vh', paddingTop: '60px', overflowX: 'hidden' }}>
        ...
        <section style={{ position: 'relative', padding: 'clamp(80px, 12vw, 160px) 0 clamp(60px, 8vw, 100px)', textAlign: 'center', overflow: 'hidden' }}>
          ...
          <div style={{ ...sectionStyle, position: 'relative' }}>
            ...
            <p style={{
              ...
              {hi.hero_sub}<br />
              <span style={{ color: white, fontWeight: 600 }}>{hi.hero_trial}</span>
            </p>
        ...
        <section style={{ padding: 'clamp(60px, 8vw, 100px) 0' }}>
          <div style={sectionStyle}>
            <div style={{ textAlign: 'center', marginBottom: '40px' }}>
              <div style={sectionHeadStyle}>{hi.partner_section_label}</div>
            </div>
                ...
                <div style={{ flex: '1 1 320px' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '8px' }}>
                    ...
                    <div>
                      ...
                      <div style={{ fontSize: '11px', fontWeight: 700, color: gold, letterSpacing: '2px', textTransform: 'uppercase' }}>{hi.partner_section_label}</div>
                    </div>
                  </div>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(120px, 1fr))', gap: '12px', marginBottom: '20px' }}>
                    {[
                      { v: '2.1M+', l: hi.partner_users_label },
                      { v: '148K',  l: hi.partner_sellers_label },
                      { v: '173K+', l: hi.partner_tx_label },
                      { v: '\$0.17', l: hi.partner_price_label },
                    ].map((st, i) => (
        ...
        <section style={{ padding: 'clamp(60px, 8vw, 100px) 0', borderTop: `1px solid ${border}` }}>
          <div style={sectionStyle}>
            <div style={{ textAlign: 'center', marginBottom: '56px' }}>
              <div style={sectionHeadStyle}>{hi.how_section_label}</div>
              <h2 style={{ ...h2Style, marginBottom: 0 }}>{hi.how_title}</h2>
            </div>
        ...
        <section style={{ padding: 'clamp(60px, 8vw, 100px) 0', borderTop: `1px solid ${border}` }}>
          <div style={sectionStyle}>
            <div style={{ textAlign: 'center', marginBottom: '48px' }}>
              <div style={sectionHeadStyle}>{hi.ladder_section_label}</div>
              <h2 style={h2Style}>{hi.ladder_title}</h2>
              <p style={{ color: muted, fontSize: '15px', maxWidth: '460px', margin: '0 auto' }}>{hi.ladder_sub}</p>
            </div>
        ...
        <section style={{ padding: 'clamp(60px, 8vw, 100px) 0', borderTop: `1px solid ${border}` }}>
          <div style={sectionStyle}>
            <div style={{ textAlign: 'center', marginBottom: '48px' }}>
              <div style={sectionHeadStyle}>{hi.pricing_section_label}</div>
              <h2 style={h2Style}>{hi.pricing_title}</h2>
              <p style={{ color: muted, fontSize: '15px' }}>{hi.pricing_sub}</p>
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(230px, 1fr))', gap: '16px' }}>
              {plans.map((plan, i) => (
                <div key={i} className="pricing-card" style={{ background: card, border: `1px solid ${plan.name === hi.plan_arena_name ? plan.color + '66' : border2}`, borderRadius: '14px', padding: 'clamp(20px, 2.5vw, 32px)', display: 'flex', flexDirection: 'column', position: 'relative', transition: 'border-color 0.2s, transform 0.2s', boxShadow: plan.name === hi.plan_arena_name ? `0 0 40px ${plan.color}22` : 'none' }}>
                  {plan.badge && (
        ...
        <section style={{ padding: 'clamp(80px, 10vw, 140px) 0', borderTop: `1px solid ${border}`, position: 'relative', overflow: 'hidden', textAlign: 'center' }}>
          ...
          <div style={{ ...sectionStyle, position: 'relative' }}>
            <div style={sectionHeadStyle}>{hi.final_section_label}</div>
            <h2 style={{ ...h2Style, fontSize: 'clamp(36px, 5vw, 60px)' }}>{hi.final_title}</h2>
            <p style={{ color: muted, fontSize: '15px', marginBottom: '36px' }}>{hi.final_sub}</p>
            ...
            <div style={{ marginTop: '20px' }}>
              <a href="/login#signin" style={{ fontSize: '14px', color: muted, textDecoration: 'none' }} className="sign-in-link">
                <span style={{ color: white, fontWeight: 600 }}>{hi.final_signin}</span>
              </a>
            </div>
          </div>
        </section>

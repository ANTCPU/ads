...
// v2 (Sep 2026) — see previous comments
// v3 (Sep 2026):
//   — FeaturedPartnerCard added above Map of Pi partner block
//   — Season 1 announcement banner added below nav
// ─────────────────────────────────────────────────────────────────────────────
...
export default function MapOfPiSplash() {
  ...
  return (
    <div style={s.page}>
      ...
      {(activeCountries !== null || totalAds !== null) && (
        <div style={{ ...s.section, borderTop: `1px solid ${border}`, textAlign: 'center' }}>
          ...
          <h2 style={{ ...s.h2, textAlign: 'center' }}>
            {activeCountries !== null ? `${activeCountries} countries` : ''}
            {activeCountries !== null && totalAds !== null ? ' · ' : ''}
            {totalAds !== null ? `${totalAds} active ads` : ''}
          </h2>
      ...
      {/* ── FEATURED PARTNER — rotates weekly via badge ── */}
      <div style={{ ...s.section, borderTop: `1px solid ${border}`, paddingBottom: 0 }}>
        <div style={{ fontSize: '0.65rem', color: gold, fontWeight: 700, letterSpacing: '0.12em', textTransform: 'uppercase', marginBottom: '0.75rem' }}>
          ⭐ Featured Partner · Season 1
        </div>

...
// Map of Pi partner stats — static, not from API
const MAP_STATS = [
  { v: '2.1M+', l: 'Users'        },
  { v: '148K',  l: 'Sellers'      },
...
// Season map — Arena 0 always live, seasons 2-4 coming
const SEASONS = [
  { n: '0', label: 'Arena 0',  icon: '⚡', color: C.orange, desc: 'The original. Always open.',        status: 'live',    href: '/arena'         },
  { n: '1', label: 'Season 1', icon: '🍂', color: C.gold,   desc: 'The Foundation. Sep 22 → Dec 21.', status: 'live',    href: '/arena/season1' },
  { n: '2', label: 'Season 2', icon: '❄️', color: C.blue,   desc: 'The Rise. Dec 21 → Mar 20.',       status: 'coming',  href: '#'              },
  { n: '3', label: 'Season 3', icon: '🌸', color: '#ff0080',desc: 'The Bloom. Mar 20 → Jun 21.',      status: 'coming',  href: '#'              },
  { n: '4', label: 'Season 4', icon: '☀️', color: C.green,  desc: 'The Peak. Jun 21 → Sep 22.',       status: 'coming',  href: '#'              },
];
...
export default function SplashPage({ locale = 'en' }: { locale?: Locale }) {
  ...
  // ─── Effects ──────────────────────────────────────────────────────────────
  useEffect(() => {
    // Locale detection — only on root /en
    if (locale === 'en') {
  ...
  // Open vault in signup mode — used by all primary CTAs
  function openSignup() {
    setVaultDefaultMode('signup');
    setVaultOpen(true);
  }

  // Open vault in signin mode — used by "Already in the Arena?" links
  function openSignin() {
    setVaultDefaultMode('signin');
    setVaultOpen(true);
  }
  // ─── Style helpers ──────────────────────────────────────────────────────────
  ...
  // Inline CSS animations — no globals needed
  const css = `
  ...
  // ── PLANS — 4 cards, all actionable ────────────────────────────────────────
  // Trial + Arena: self-serve Path A (opens VaultModal signup)
  // Place My Ad:   Path B — email capture, ANTCPU manages the ad
  // Cloud:         links to antcpu.com/cloud/#apply
  // No disabled cards. No dead ends.

  const PLANS = [
    {
      name:       t(activeLocale, 'plan_trial_name'),
      price:      t(activeLocale, 'plan_trial_price'),
      period:     t(activeLocale, 'plan_trial_period'),
      color:      C.teal,
      badge:      '',
      badgeColor: '',
      features: [
        t(activeLocale, 'plan_trial_f1'),
        t(activeLocale, 'plan_trial_f2'),
        t(activeLocale, 'plan_trial_f3'),
        t(activeLocale, 'plan_trial_f4'),
      ],
      cta:      t(activeLocale, 'plan_trial_cta'),
      // Path A — opens signup modal inline
      action:   'signup' as const,
      href:     null,
    },
    {
      name:       t(activeLocale, 'plan_arena_name'),
      price:      t(activeLocale, 'plan_arena_price'),
      period:     t(activeLocale, 'plan_arena_period'),
      color:      C.orange,
      badge:      t(activeLocale, 'plan_arena_badge'),
      badgeColor: C.orange,
      features: [
        t(activeLocale, 'plan_arena_f1'),
        t(activeLocale, 'plan_arena_f2'),
        t(activeLocale, 'plan_arena_f3'),
        t(activeLocale, 'plan_arena_f4'),
        t(activeLocale, 'plan_arena_f5'),
      ],
      cta:    t(activeLocale, 'plan_arena_cta'),
      action: 'signup' as const,
      href:   null,
    },
    {
      // Path B — placement client. No dashboard. ANTCPU manages the ad.
      // Email capture only — links to /place (new route, Phase 4)
      // Until /place exists, links to antcpu.com/cloud/#apply as interim
      name:       'Place My Ad',
      price:      'Free',
      period:     'to start',
      color:      C.purple,
      ...
      features: [
        ...
        '✓ ANTCPU places it for you',
        '✓ Weekly email update',
        '✓ No login required',
        '✓ Soft nudge when ad ends',
      ],
      cta:    'Place My Ad →',
      action: 'href' as const,
      href:   'https://antcpu.com/cloud/#apply',
    },
    {
      // Cloud — invite only, 10 antbots, full network
      name:       'Cloud',
      price:      '$20+',
      period:     'per month',
      color:      C.gold,
      ...
      badgeColor: C.gold,
      features: [
        '✓ 10 AI antbots running 24/7',
        '✓ 8 languages, global reach',
        '✓ Full network distribution',
        '✓ Weekly performance report',
        '✓ Creator channel integrations',
      ],
      cta:    'Apply for Cloud →',
      action: 'href' as const,
      href:   'https://antcpu.com/cloud/#apply',
...
<a href="/arena/season1" style={{
  ...
  🍂 Season 1
          ...
          {/* Sign In — opens vault in signin mode */}
          <button onClick={openSignin} className="nav-a"
          ...
          {/* Primary nav CTA — opens vault in signup mode, no page redirect */}
          <button onClick={openSignup} className="cta"
      ...
      <div style={{ paddingTop: 60 }}>
        ...
        <section style={{
          ...
          <div className="hero-in" style={{ ...sec, width: '100%' }}>
            ...
            {/* CTAs — both open VaultModal, no page redirect */}
            <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', marginBottom: 16 }}>

              {/* Primary — signup */}
              <button onClick={openSignup} className="cta" style={btn(C.orange)}>
                {t(activeLocale, 'hero_cta_primary')}
              </button>

              {/* Secondary — browse arena, no account needed */}
              <a href="/arena" className="ghost" style={ghostBtn}>
                {t(activeLocale, 'hero_cta_secondary')}
              </a>
            </div>

            {/* No-pressure sub-copy — no mention of "3 days then $9.99" */}
        ...
        <section style={{ ...pad, borderBottom: `1px solid ${C.border}` }}>
          <div style={sec}>
            ...
            {/* One more click — inline signup nudge below steps */}
            <div style={{
              marginTop:      32,
              display:        'flex',
              alignItems:     'center',
              ...
              <div style={{ flex: 1, minWidth: 200 }}>
                ...
                <div style={{ fontSize: 13, color: C.muted }}>
                  Name, brand, ad. That&apos;s it. Aria reviews it. You go live same day.
                </div>
              </div>
              <button onClick={openSignup} className="cta"
                style={{ ...btn(C.orange), padding: '10px 22px', fontSize: 13, flexShrink: 0 }}>
                Join Free →
              </button>
            </div>
          </div>
        ...
        <section style={{
          ...
          <div style={sec}>
            ...
            {/* One more click — ladder bottom nudge */}
            <div style={{
              marginTop:    24,
              textAlign:    'center',
              padding:      '18px',
              ...
              <span style={{ fontSize: 13, color: C.muted }}>
                Every action earns points. Start at Entry — climb from there.{' '}
              </span>
              <button
                onClick={openSignup}
                style={{
              ...
              {PLANS.map((plan, i) => (
                <div key={i} className="card-hover"
                  style={{
                    ...
                    // Highlight the Most Popular card (Arena, index 1)
                    boxShadow:      i === 1 ? `0 0 0 1px ${C.orange}40` : 'none',
                  }}>
                  ...
                  {/* CTA — signup opens modal, href navigates out */}
                  {plan.action === 'signup' ? (
                    <button
                      onClick={openSignup}
                      className="cta"
        ...
        <section style={{
          ...
          <div style={sec}>
            ...
            <p style={{
              ...
              Seasonal arenas open every 90 days. Fresh start. New champion
              crowned at the close. Every brand can earn their own sub-arena
              through shares.
            </p>
              ...
              {SEASONS.map((s) => {
                ...
                return (
                  <a
                    ...
                    style={{
                      background:     C.card,
                      border:         `1px solid ${isLive ? s.color + '40' : C.border}`,
                    ...
                    {/* Live pulse — both Arena 0 and Season 1 are live */}
                    {isLive && (
                      <div style={{
                        position:     'absolute',
                        top:          12,
            ...
            <p style={{ fontSize: 12, color: C.muted2, marginTop: 16 }}>
              Every brand earns their own arena. Share your ad to build your
              audience. →{' '}
              <a href="/arena"
        ...
        <section style={{ ...pad, borderBottom: `1px solid ${C.border}` }}>
          <div style={sec}>
            ...
            {/* motivatorText updated — Season 1 is live, not "opens next week" */}
            <FeaturedPartnerCard
              season="Season 1"
              motivatorText="Top brands earn this spotlight. Compete in Season 1 for the featured slot."
            />

            {/* ── Permanent Map of Pi founding partner block ── */}
            <div style={{
              background:  C.card,
              ...
              <p style={{
                ...
                The world&apos;s largest Pi commerce platform — connecting buyers
                and sellers across 88 countries using Pi Network. Every Map of Pi
                user in the Arena is a Country Champion representing their nation.
              </p>
        ...
        <section style={{ ...pad }}>
          <div style={{ ...sec, textAlign: 'center' }}>
            ...
            <div style={{
              ...
              {/* Primary — inline signup, no redirect */}
              <button
                onClick={openSignup}
                className="cta"
              ...
              {/* Season 1 — direct link, already live */}
        ...
        <footer style={{
          ...
          <div style={{ display: 'flex', gap: 24, flexWrap: 'wrap', alignItems: 'center' }}>
            ...
            {/* Footer CTA — last thing a visitor sees before leaving */}
            <button
              onClick={openSignup}
              className="cta"
              style={{
      ...
      {/* ── VAULT MODAL ── */}
      {/* defaultMode driven by vaultDefaultMode state */}
      {/* openSignup() sets 'signup', openSignin() sets 'signin' */}
      <VaultModal
        open={vaultOpen}
        onClose={() => setVaultOpen(false)}
        onSuccess={() => {}}
        defaultMode={vaultDefaultMode}
      />

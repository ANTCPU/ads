...
// All badge operations flow through here — award and revoke, any tier.
//
// Award source is always: "Ad Arena Badge System"
// Never a person's name or email — consistent, trustworthy, system identity.
//
// Auth: CRON_SECRET header OR viewer email matches NEXT_PUBLIC_SUPER_EMAIL.
// Both paths are server-verified — client cannot spoof.
//
// GET    ?email=x
//   → returns current badges for that user
//   → used by users page badge section on row expand
//
// POST   { email, badge_slug, viewer_email }
//   → awards any badge, any tier
//   → awardedBy = 'Ad Arena Badge System' always
//   → writes to user_badges (idempotent)
//   → fires in-app notification to user
//   → Discord ping
//
// DELETE { email, badge_slug, viewer_email, reason? }
//   → revokes any badge, any tier
//   → writes to badge_revocations (audit log)
//   → checks revocation count → triggers consequence chain:
//       1 revocation  → in-app nudge only
//       2 revocations → ads set to pending_review
//       3+ revocations → account suspended_review, ads archived
//   → Discord ping with severity
//
// Consequence chain is productive, not punitive.
// User always sees "under review" — never a reason, never public shame.
//
// v1 (Sep 2026): initial — award + revoke + consequence chain
// ─────────────────────────────────────────────────────────────────────────────
...
// ─── Auth ─────────────────────────────────────────────────────────────────────
// Accepts cron secret header OR super email in body.
// Never trusts role from client — always server-verified.
...
// ─── Consequence chain ────────────────────────────────────────────────────────
// Called after every revocation.
// Checks total revocation count for this email and escalates accordingly.
// Always productive — user sees "under review", never a reason.
...
async function applyConsequenceChain(
  ...
  // Count total revocations for this email
  ...
  // ── Level 1 — first revocation — nudge only ───────────────────────────────
  if (total === 1) {
    await notifyUser(
      email, 'info',
    ...
    await notifyDiscord('', 'general', {
      ...
      footer:    'Ad Arena Badge System · Consequence Chain',
      timestamp: true,
    });
  ...
  // ── Level 2 — second revocation — ads to pending_review ──────────────────
  if (total === 2) {
    await supabase
      .from('ads')
  ...
  // ── Level 3+ — repeat offender — account suspended ───────────────────────
  await supabase
    .from('ads')
  ...
  await supabase
    .from('ad_signups')
    .update({ status: 'suspended_review' })
    .eq('email', email);

  await notifyUser(
    email, 'info',
    '🦋 Your account is under review',
    'Your account and ads aren\'t visible right now. ' +
    'We\'ll reach out directly. Your data is safe.',
  );

  await notifyDiscord('', 'general', {
    title:  '🚨 Badge Revoked — Level 3+ SUSPENDED',
    color:  0xef4444,
    fields: [
      { name: 'Email',  value: email,          inline: true  },
      { name: 'Badge',  value: badgeSlug,       inline: true  },
...
// ─── GET — fetch current badges for a user ────────────────────────────────────
...
export async function POST(req: NextRequest) {
  try {
    ...
    // Award — awardedBy is always the system identity
    const awarded = await awardBadge(
      supabase, norm, badge_slug as BadgeSlug, AWARD_SOURCE
    );
    ...
    await notifyUser(
      ...
      def.slug === 'featured-profile'
        ...
        : def.slug === 'arena-original'
        ? `🔥 You joined before 100 members. There is only one Arena — and you were here first.`
        : `${def.desc}`,
    );

    // Discord ping
    notifyDiscord('', 'general', {
      title:  `🏅 Badge Awarded — ${def.label}`,
      color:  DC.green,
      fields: [
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    // Write to audit log BEFORE consequence chain
    // (chain reads count — must include this revocation)

...
export default function MacPage() {
  ...
  const card   = '#111';
  const border = '#1a1a1a';
  const green  = '#22c55e';   // MAC brand colour
  const white  = '#fff';
  const muted  = '#555';
  ...
  useEffect(() => {
    ...
    // Language from arena locale
    const lang = getStoredLocale();
    setLanguage(lang);

    // User + country from localStorage
    try {
    ...
    // Country from ipapi if not in user record
  ...
  async function send(text: string) {
    ...
    try {
      ...
      // Surface champion slot on first response
      if (data.champion_slot_open && !championOpen) setChampionOpen(true);

      // Store ad draft when ready

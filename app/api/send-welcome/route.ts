...
//
// If gate blocks → in-app notification + skip log + Discord gated ping
// If gate passes → heraldSend + recordEmailSent + logEmailSend + Discord
//
// v4 (Sep 2026):
//   — logEmailSend added — send log populates for Scout monthly report
//   — logSkippedSend added — gate blocks recorded with reason + segment
...
//   — Discord pings: plain text → rich embed
...
function dashboardUrl(role: string, email: string): string {
  if (role === 'super' || (SUPER_EMAIL && email === SUPER_EMAIL))
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!gate.allow) {
      ...
      try {
        await logSkippedSend(supabase, email, 'welcome', gate.reason, {
    ...
    try {
      await logEmailSend(supabase, email, 'welcome', {
        segment: source || 'organic',
        subject,
        locale,
        status:  'sent',

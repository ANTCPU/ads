  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const res  = await fetch('/api/support', {
        ...
        body: JSON.stringify({ email, name, subject: issueEn, message, locale }),
      });
            ...
            {!myAd && (
              <div style={{ ...card, border: '1px solid #0070f340', background: '#0070f308' }}>
                ...
                <div style={{ fontSize: '0.78rem', color: '#888', marginBottom: '1rem' }}>
                  Create your first ad and get it in front of the Arena.
                </div>
                <button onClick={() => router.push('/create-ad')} style={btn('#0070f3')}>
                  Create Ad →
                </button>
              </div>
            )}

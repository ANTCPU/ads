  ...
  return (
    <div style={{ background: '#fff', color: '#0a0a0a', fontFamily: 'system-ui, sans-serif', minHeight: '100vh' }}>
      ...
      <div style={{ maxWidth: '600px', margin: '0 auto', padding: '2rem 1.5rem' }}>
        ...
        {/* HEADER */}
        <SectionHeader title="🗺️ Map of Pi" sub="Brand dashboard — post builder + content tools" />
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.5rem', marginBottom: '1.5rem' }}>
          ...
          <Pill label="🏟 Arena" onClick={() => router.push('/arena/mapofpi')} color={accent} outline />
          <Pill label="🌐 Map of Pi" onClick={() => router.push('/mapofpi')} color="#D4AF37" />
          <Pill label="📋 Copy Referral" onClick={() => { navigator.clipboard.writeText('https://antcpu-ads.vercel.app/mapofpi?promo=MAPOFPI'); setCopiedId(-1); setTimeout(() => setCopiedId(null), 2500); }} color={copiedId === -1 ? '#22c55e' : '#D4AF37'} outline />
        ...
        <div style={{ background: '#f9f9f9', border: '1px solid #e5e5e5', borderRadius: '10px', padding: '0.85rem 1rem', marginBottom: '1.5rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div style={{ fontSize: '0.75rem', color: '#888' }}>🚀 Live Pi Price</div>
          <div style={{ fontWeight: 800, fontSize: '1.2rem', color: '#D4AF37' }}>{piPrice}</div>
        </div>
        ...
        {TEAM.map(m => (
          <Card key={m.email}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
              ...
              <span style={{ marginLeft: 'auto', fontSize: '0.7rem', background: '#f0fdf4', color: '#16a34a', border: '1px solid #bbf7d0', borderRadius: '999px', padding: '0.15rem 0.6rem', fontWeight: 700 }}>✅ Team</span>
            </div>
          </Card>
        ))}

        {/* POST BUILDER */}
        <SectionHeader title="📋 Post Builder" sub="Copy and post to your socials" />
        {POSTS.map(p => (
          <Card key={p.id}>
            <div style={{ fontSize: '0.7rem', color: '#888', marginBottom: '0.5rem', fontWeight: 600 }}>{p.tag}</div>
            <pre style={{ fontSize: '0.82rem', whiteSpace: 'pre-wrap', margin: '0 0 0.75rem', color: '#0a0a0a', fontFamily: 'system-ui, sans-serif' }}>{p.text}</pre>
            <Pill label={copiedId === p.id ? '✅ Copied' : '📋 Copy'} onClick={() => copyPost(p.text, p.id)} color={copiedId === p.id ? '#22c55e' : accent} />
          </Card>
        ))}

        {/* CUSTOM POST */}
        <SectionHeader title="✏️ Custom Post" sub="Write your own and copy it" />
        <textarea
          value={custom}
          onChange={e => setCustom(e.target.value)}
          placeholder="Write a custom post..."
          style={{ width: '100%', minHeight: '120px', background: '#fafafa', border: '1px solid #e5e5e5', borderRadius: '8px', padding: '0.75rem', fontSize: '0.85rem', color: '#0a0a0a', outline: 'none', resize: 'vertical', boxSizing: 'border-box', marginBottom: '0.75rem' }}
        />
        <Pill label={customCopied ? '✅ Copied' : '📋 Copy Custom'} onClick={copyCustom} color={customCopied ? '#22c55e' : accent} />

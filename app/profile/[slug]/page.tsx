'use client';
// app/profile/[slug]/ProfileClient.tsx
...
type ProfilePill = {
  ...
  href: string; guidePath: string;
  minTier: MembershipTier; flagId: string | null;
};
...
function getPillState(
  pill: ProfilePill, membershipTier: MembershipTier,
  flagMap: Record<string, boolean>, isSuper: boolean,
): PillState {
...
const bg = tokens.bg; const card = tokens.card; const border = tokens.border;
const muted = '#555'; const white = '#fff'; const gold = '#D4AF37';
...
function getYouTubeEmbedUrl(url: string): string {
  ...
  return `https://www.youtube.com/embed/KuSW1xT8jmo?autoplay=0&rel=0`;
}
...
export default function ProfileClient() {
  ...
  useEffect(() => {
    ...
    if (stored) { try { v = JSON.parse(stored); setViewer(v); } catch {} }
    ...
    Promise.all([
      ...
      supabase.from('user_badges').select('badge_slug, awarded_at')
      ...
      if (profileRes.data) {
        ...
        setForm(loaded); setOrigForm(loaded);
        if (profileRes.data.bio) setHasProfile(true);
      } else if (isOwner) { setEditing(true); }
      if (signupRes.data) {
  ...
  async function saveAdEdit(id: string) {
    ...
    await supabase.from('ads').update({
      title: adEditForm.title.trim(), description: adEditForm.description.trim(), url: adEditForm.url.trim(),
    }).eq('id', id);
  ...
  async function handleAwardBadge() {
    ...
    if (data.ok) setBadges(prev => [...prev, { badge_slug: awardingBadge, awarded_at: new Date().toISOString() }]);
  ...
  async function handleOverrideTier() {
    ...
    setOverrideResult('…');
    const { error } = await supabase.from('ad_signups').update({ membership_tier: overrideTier }).eq('email', profileEmail);
  ...
  const cardStyle: React.CSSProperties = { background: card, border: `1px solid ${border}`, borderRadius: '12px', padding: '1.25rem', marginBottom: '1rem' };
  const lbl: React.CSSProperties       = { fontSize: '0.68rem', color: muted, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: '0.75rem' };
  const inp: React.CSSProperties       = { ...baseInp, background: bg, marginBottom: '0.75rem', fontSize: '0.88rem', padding: '0.65rem 0.85rem' };
  const rowBtn = (color: string, disabled = false): React.CSSProperties => ({
    ...
    cursor: disabled ? 'default' : 'pointer', whiteSpace: 'nowrap' as const, transition: 'all 0.15s',
  });
  ...
  return (
    <div style={{ background: bg, minHeight: '100vh', color: white, fontFamily: 'system-ui, sans-serif' }}>
      ...
      <div style={{ maxWidth: '600px', margin: '0 auto', padding: '2rem 1.25rem' }}>
        ...
        <button onClick={() => router.back()} style={{ background: 'none', border: 'none', color: muted, cursor: 'pointer', fontSize: '0.78rem', padding: 0, marginBottom: '1.5rem' }}>← Back</button>
          ...
          <div style={{ display: 'flex', gap: '0.4rem', marginBottom: '1.25rem', background: card, border: `1px solid ${border}`, borderRadius: '10px', padding: '0.5rem' }}>
            {(['admin', 'owner', 'public'] as ViewMode[]).map(mode => (
              <button key={mode} onClick={() => setViewMode(mode)} style={{
                ...
                fontWeight: 700, fontSize: '0.72rem', padding: '0.4rem 0', cursor: 'pointer', transition: 'all 0.15s',
              }}>
        ...
        <div style={{ ...cardStyle, position: 'relative', overflow: 'hidden' }}>
          ...
          <div style={{ marginBottom: '0.75rem' }}>
            <div style={{ fontWeight: 800, fontSize: '1.1rem', color: white, marginBottom: '0.2rem' }}>
              {form.bio ? (viewer?.name || profileEmail.split('@')[0]) : profileEmail.split('@')[0]}
            </div>
            <div style={{ fontSize: '0.78rem', color: muted, display: 'flex', gap: '0.5rem', alignItems: 'center', flexWrap: 'wrap' }}>
              {viewer?.brand && <span style={{ color: accent, fontWeight: 700 }}>{viewer.brand}</span>}
              {viewer?.brand && <span>·</span>}
              <span style={{ background: `${tierDef.color}15`, border: `1px solid ${tierDef.color}40`, color: tierDef.color, borderRadius: '999px', padding: '0.1rem 0.5rem', fontSize: '0.65rem', fontWeight: 700 }}>
                {tierDef.icon} {tierDef.label}
              </span>
              {knownBadges.some(b => b.badge_slug === 'country-champion') && (
                <span style={{ background: `${gold}15`, border: `1px solid ${gold}40`, color: gold, borderRadius: '999px', padding: '0.1rem 0.5rem', fontSize: '0.65rem', fontWeight: 700 }}>🏆 Champion</span>
              )}
            </div>
          </div>
          ...
          <div style={{ display: 'flex', gap: '0.4rem', flexWrap: 'wrap' }}>
            {PROFILE_PILLS.map(pill => {
              ...
              return (
                <button key={pill.id} onClick={() => dest && router.push(dest)}
                  title={state === 'guide' ? `Unlock at ${pill.minTier} tier — tap to learn how` : state === 'soon' ? 'Coming soon' : pill.label}
                  style={{
                    background: state === 'active' ? `${accent}15` : 'transparent',
                    border: `1px ${state === 'guide' ? 'dashed' : 'solid'} ${pillAccent}`,
                    borderRadius: '999px', color: state === 'active' ? accent : state === 'soon' ? '#333' : '#444',
                    fontSize: '0.72rem', fontWeight: 700, padding: '0.3rem 0.75rem',
                    cursor: state === 'soon' ? 'default' : 'pointer', opacity: state === 'soon' ? 0.5 : 1,
          ...
          <div style={cardStyle}>
            ...
            <div style={{ display: 'flex', gap: '0.4rem', flexWrap: 'wrap' }}>
              {knownBadges.map(b => {
                ...
                const defColor = def.tier === 4 ? gold : def.tier === 3 ? '#22c55e' : def.tier === 2 ? '#0070f3' : '#7928ca';
                return (
        ...
        {hasProfile && !editing && (
          <>
            <div style={cardStyle}>
              ...
              {form.contact && <div style={{ fontSize: '0.82rem', color: muted, marginTop: '0.5rem' }}>📧 {form.contact}</div>}
              {form.preferred_locale !== 'en' && (
                <div style={{ fontSize: '0.78rem', color: muted, marginTop: '0.5rem' }}>
                  🤖 Agent language: <strong style={{ color: accent }}>{LANGUAGES.find(l => l.code === form.preferred_locale)?.name || 'English'}</strong>
                </div>
              )}
            </div>
            ...
            <div style={cardStyle}>
              ...
              <iframe src={getYouTubeEmbedUrl(form.youtube || '')} width="100%" height="200" style={{ borderRadius: '10px', border: 'none' }} allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope" allowFullScreen />
            </div>
            ...
            {effectiveOwner && (
              <button onClick={() => setEditing(true)} style={{ width: '100%', background: accent, border: 'none', color: '#fff', borderRadius: '10px', padding: '0.85rem', fontWeight: 800, fontSize: '0.9rem', cursor: 'pointer', marginBottom: '1rem' }}>
                ✏️ Edit Profile
              </button>
            )}
          </>
        ...
        {editing && effectiveOwner && (
          <>
            <div style={cardStyle}>
              ...
              <textarea value={form.bio} onChange={e => setForm(f => ({ ...f, bio: e.target.value }))} placeholder="Tell the Arena who you are..." rows={4} style={{ ...inp, resize: 'vertical', width: '100%', boxSizing: 'border-box' }} />
              <div style={lbl}>Contact Email</div>
              <input value={form.contact} onChange={e => setForm(f => ({ ...f, contact: e.target.value }))} placeholder="hello@yourbrand.com" style={{ ...inp, width: '100%', boxSizing: 'border-box' }} />
            </div>

            <div style={cardStyle}>
              <div style={lbl}>🤖 Agent Language</div>
              <div style={{ fontSize: '0.78rem', color: muted, marginBottom: '0.75rem' }}>Aria and your antbots will write in this language.</div>
              <div style={{ display: 'flex', gap: '0.4rem', flexWrap: 'wrap' }}>
                {LANGUAGES.map(l => (
                  <button key={l.code} onClick={() => setForm(f => ({ ...f, preferred_locale: l.code }))} style={{
                    ...
                    borderRadius: '8px', padding: '0.35rem 0.75rem', fontSize: '0.78rem', fontWeight: 700, cursor: 'pointer',
                  }}>{l.label}</button>
                ))}
              </div>
            </div>
            ...
            <div style={cardStyle}>
              ...
              {SOCIALS.map(s => (
                <div key={s.key}>
                  ...
                  <input value={form[s.key] as string} onChange={e => setForm(f => ({ ...f, [s.key]: e.target.value }))} placeholder={s.placeholder} style={{ ...inp, width: '100%', boxSizing: 'border-box' }} />
                </div>
              ))}
            </div>

            <div style={{ display: 'flex', gap: '0.75rem', marginBottom: '1rem' }}>
              <button onClick={handleSave} disabled={saving} style={{ flex: 1, background: saved ? '#22c55e' : accent, border: 'none', color: '#fff', borderRadius: '10px', padding: '0.85rem', fontWeight: 800, fontSize: '0.9rem', cursor: saving ? 'not-allowed' : 'pointer' }}>
                {saving ? 'Saving...' : saved ? '✅ Saved' : 'Save Profile'}
              </button>
              {hasProfile && (
                <button onClick={() => setEditing(false)} style={{ background: 'transparent', border: `1px solid ${border}`, color: muted, borderRadius: '10px', padding: '0.85rem 1.25rem', fontWeight: 700, fontSize: '0.9rem', cursor: 'pointer' }}>
                  Cancel
                </button>
              )}
            </div>
        ...
        {(effectiveOwner ? ads.length > 0 : activeAds.length > 0) && (
          <div style={cardStyle}>
            ...
            {visibleAds.length === 0 ? (
              <div style={{ fontSize: '0.82rem', color: muted, padding: '0.5rem 0' }}>No {adsTab} ads.</div>
            ) : (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                {visibleAds.map(ad => {
                  ...
                  return (
                    <div key={ad.id} style={{ background: bg, border: `1px solid ${border}`, borderLeft: `3px solid ${tierColor}`, borderRadius: '10px', padding: '1rem' }}>
                      <div style={{ display: 'flex', gap: '0.4rem', flexWrap: 'wrap', alignItems: 'center', marginBottom: '0.4rem' }}>
                        ...
                        {ad.is_country_champion && ad.country && <span style={{ fontSize: '0.62rem', color: gold, fontWeight: 700 }}>🏆 {ad.country}</span>}
                        {ad.rank_position && ad.rank_position <= 3 && <span>{ad.rank_position === 1 ? '🥇' : ad.rank_position === 2 ? '🥈' : '🥉'}</span>}
                      </div>
                      ...
                      {isEditing && (
                        <div style={{ marginBottom: '0.65rem' }}>
                          <input value={adEditForm.title} onChange={e => setAdEditForm(f => ({ ...f, title: e.target.value }))} placeholder="Title" style={{ ...inp, width: '100%', boxSizing: 'border-box' }} />
                          <textarea value={adEditForm.description} onChange={e => setAdEditForm(f => ({ ...f, description: e.target.value }))} placeholder="Description" rows={3} style={{ ...inp, resize: 'vertical', width: '100%', boxSizing: 'border-box' }} />
                          <input value={adEditForm.url} onChange={e => setAdEditForm(f => ({ ...f, url: e.target.value }))} placeholder="URL" style={{ ...inp, width: '100%', boxSizing: 'border-box', marginBottom: 0 }} />
                        </div>
                      )}
                      ...
                      <div style={{ display: 'flex', gap: '0.4rem', flexWrap: 'wrap' }}>
                        {!isEditing && ad.url && <button onClick={() => window.open(ad.url, '_blank', 'noopener,noreferrer')} style={rowBtn(tierColor)}>Visit →</button>}
                        ...
                        {isEditing && (
                          <>
                            <button onClick={() => saveAdEdit(ad.id)} disabled={adSaving} style={rowBtn('#22c55e', adSaving)}>{adSaving ? '…' : '💾 Save'}</button>
        ...
        {effectiveAdmin && (
          <div style={{ ...cardStyle, border: `1px solid ${accent}30` }}>
            ...
            <div style={{ marginBottom: '1rem' }}>
              <div style={{ fontSize: '0.72rem', color: muted, marginBottom: '0.4rem', fontWeight: 700 }}>Override Membership Tier</div>
              <div style={{ display: 'flex', gap: '0.4rem', flexWrap: 'wrap', marginBottom: '0.5rem' }}>
                {MEMBERSHIP_TIERS.map(t => (
                  <button key={t.key} onClick={() => setOverrideTier(t.key as MembershipTier)} style={{
                    ...
                    fontSize: '0.68rem', fontWeight: 700, padding: '0.3rem 0.65rem', cursor: 'pointer',
                  }}>{t.icon} {t.label}</button>
                ))}
              </div>
              <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
                <button onClick={handleOverrideTier} disabled={!overrideTier} style={rowBtn(accent, !overrideTier)}>Apply Tier</button>
                {overrideResult && <span style={{ fontSize: '0.78rem', color: overrideResult.startsWith('✅') ? '#22c55e' : '#ef4444' }}>{overrideResult}</span>}
              </div>
            </div>

            <div style={{ marginBottom: '1rem' }}>
              <div style={{ fontSize: '0.72rem', color: muted, marginBottom: '0.4rem', fontWeight: 700 }}>Award Badge</div>
              <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap', marginBottom: '0.5rem' }}>
                {BADGE_REGISTRY.map(b => (
                  <button key={b.slug} onClick={() => setAwardingBadge(b.slug)} style={{
                    ...
                    fontSize: '0.65rem', fontWeight: 700, padding: '0.2rem 0.55rem', cursor: 'pointer',
                  }}>{b.icon} {b.label}</button>
                ))}
              </div>
              <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
                <button onClick={handleAwardBadge} disabled={!awardingBadge} style={rowBtn(gold, !awardingBadge)}>Award Badge</button>
        ...
        {effectiveOwner && (
          <div style={{ ...cardStyle, display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '0.75rem' }}>
            <div>
              ...
              <div style={{ fontSize: '0.75rem', color: muted }}>antcpu-ads.vercel.app/profile/{slug}</div>
            </div>
            <button onClick={() => navigator.clipboard.writeText(`https://antcpu-ads.vercel.app/profile/${slug}`)} style={rowBtn(accent)}>📋 Copy</button>
          </div>
        )}
